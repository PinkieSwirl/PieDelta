...
internal value class DeltaCreatorChecksum(internal val checksum: Long = 0) {
    ...
    internal fun increment(old: Byte, new: Byte, chunkSize: Int): DeltaCreatorChecksum {
        val old1 = old.substitute()
        val low = (((checksum and MASK_L).toInt() - old1) + new.substitute()) and MASK_I
        val high = (((checksum shr SHIFT).toInt() - (old1 * chunkSize)) + low) and MASK_I
        return DeltaCreatorChecksum((high shl SHIFT or low).toLong())
    }
...
internal value class Checksums(val checksums: DeltaCreatorMap = HashDeltaCreatorMap()) {

    inline fun indexOf(checksum: DeltaCreatorChecksum): Int = checksums.getOrDefault(checksum.checksum, -1)

    companion object {

        fun SeekableByteChannel.computeChecksumsFromByteSource(chunkSize: Int): Checksums {
            val checksums = HashDeltaCreatorMap()
            val buffer =  ByteBuffer.allocate(chunkSize * 2)
            var count = 0
            while (true) {
                read(buffer)
                buffer.flip()
                if (buffer.remaining() < chunkSize) break
                while (buffer.remaining() >= chunkSize) {
                    checksums.putIfAbsent(buffer.computeChecksumInternal(chunkSize).checksum, count++)
                }
                buffer.compact()
            }
...
internal fun ByteBuffer.computeChecksum(chunkSize: Int): DeltaCreatorChecksum {
    mark()
    val checksum = computeChecksumInternal(chunkSize)
    reset()
    return checksum
}

internal fun ByteBuffer.computeChecksumInternal(chunkSize: Int): DeltaCreatorChecksum {
    var high = 0
    var low = 0
    repeat(chunkSize) {
        low += get().substitute()
    ...
    return DeltaCreatorChecksum(((high and MASK_I) shl SHIFT or (low and MASK_I)).toLong())
}
